#include "../../include/cuda/cudaTypeDef.cuh"
...
namespace sl {
    namespace restructor {
        namespace cudaFunc {
            ...
            void depthMap(const cv::cuda::GpuMat &leftImg_, const cv::cuda::GpuMat &rightImg_,
                          const int rows, const int cols,
                          const int minDisparity, const int maxDisparity,
                          const float minDepth, const float maxDepth,
                          const Eigen::Matrix4f &Q, const Eigen::Matrix3f &M1,
                ...
                cudaStream_t stream = cv::cuda::StreamAccessor::getStream(cvStream);
                dim3 grid((cols + block.x - 1) / block.x, (rows + block.y - 1) / block.y);
                matchAndTriangulate_CUDA<<<grid, block, 0, stream>>>(
                        leftImg_, rightImg_,
                        rows, cols,
                        minDisparity, maxDisparity,
                        minDepth, maxDepth,
