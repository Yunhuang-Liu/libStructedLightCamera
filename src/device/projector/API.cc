...
#include "../../../include/device/projector/API.h"
#include <string>
#include "../../../include/device/projector/usb.h"
#include "../../../include/device/projector/common.h"
#include <stdlib.h>
#include <stdio.h>
...
CmdFormat CmdList[] =
{
    {   0x00, 0x1A,  0x00,  true, 0x01, (char*)"SOURCE_SEL" },      //SOURCE_SEL,
    {   0x02, 0x1A,  0x02,  true, 0x01, (char*)"PIXEL_FORMAT"   },      //PIXEL_FORMAT,
    {   0x03, 0x1A,  0x03,  true, 0x01, (char*)"CLK_SEL"   },      //CLK_SEL,
    {   0x04, 0x1A,  0x37,  true, 0x01, (char*)"CHANNEL_SWAP"   },      //CHANNEL_SWAP,
    {   0x05, 0x1A,  0x04,  true, 0x01, (char*)"FPD_MODE"   },      //FPD_MODE,
    {   0x07, 0x02,  0x00,  true, 0x01, (char*)"POWER_CONTROL"  },      //POWER_CONTROL,
    {   0x08, 0x10,  0x08,  true, 0x01, (char*)"FLIP_LONG"  },      //FLIP_LONG,
    {   0x09, 0x10,  0x09,  true, 0x01, (char*)"FLIP_SHORT"   },      //FLIP_SHORT,
    {   0x0A, 0x12,  0x03,  true, 0x01, (char*)"TPG_SEL"   },      //TPG_SEL,
    {   0x0B, 0x1A,  0x05,  true, 0x01, (char*)"PWM_INVERT"   },      //PWM_INVERT,
    {   0x10, 0x1A,  0x07,  true, 0x01, (char*)"LED_ENABLE"   },      //LED_ENABLE,
    {   0x11, 0x02,  0x05,  false, 0x00,(char*)"GET_VERSION"   },      //GET_VERSION,
    {   0x13, 0x08,  0x02,  true, 0x01, (char*)"SW_RESET"   },      //SW_RESET,
    {   0x20, 0x1A,  0x0A,  false, 0x00,(char*)"STATUS_HW"   },      //STATUS_HW,
    {   0x21, 0x1A,  0x0B,  false, 0x00,(char*)"STATUS_SYS"   },      //STATUS_SYS,
    {   0x22, 0x1A,  0x0C,  false, 0x00,(char*)"STATUS_MAIN"   },      //STATUS_MAIN,
    {   0x40, 0x1A,  0x10,  true, 0x01, (char*)"PWM_ENABLE"   },      //PWM_ENABLE,
    {   0x41, 0x1A,  0x11,  true, 0x06, (char*)"PWM_SETUP"   },      //PWM_SETUP,
    {   0x43, 0x1A,  0x12,  true, 0x05, (char*)"PWM_CAPTURE_CONFIG"   },      //PWM_CAPTURE_CONFIG,
    {   0x44, 0x1A,  0x38,  true, 0x02, (char*)"GPIO_CONFIG"   },      //GPIO_CONFIG,
    {   0x4B, 0x0B,  0x01,  true, 0x03, (char*)"LED_CURRENT"   },      //LED_CURRENT,
    {   0x7E, 0x10,  0x00,  true, 0x10 ,(char*)"DISP_CONFIG"  },      //DISP_CONFIG,
    {   0x69, 0x1A,  0x1B,  true, 0x01, (char*)"DISP_MODE"   },      //DISP_MODE,
    {   0x6A, 0x1A,  0x1D,  true, 0x05, (char*)"TRIG_OUT1_CTL"   },      //TRIG_OUT1_CTL,
    {   0x6B, 0x1A,  0x1E,  true, 0x05, (char*)"TRIG_OUT2_CTL"   },      //TRIG_OUT2_CTL,
    {   0x6C, 0x1A,  0x1F,  true, 0x04, (char*)"RED_LED_ENABLE_DLY"   },  //RED_LED_ENABLE_DLY,
    {   0x6D, 0x1A,  0x20,  true, 0x04, (char*)"GREEN_LED_ENABLE_DLY"   }, //GREEN_LED_ENABLE_DLY,
    {   0x6E, 0x1A,  0x21,  true, 0x04, (char*)"BLUE_LED_ENABLE_DLY"   }, //BLUE_LED_ENABLE_DLY,
    {   0x65, 0x1A,  0x24,  true, 0x01, (char*)"PAT_START_STOP"   },      //PAT_START_STOP,
    {   0x79, 0x1A,  0x35,  true, 0x03, (char*)"TRIG_IN1_CTL"},      //TRIG_IN1_CTL,
    {   0x7A, 0x1A,  0x36,  true, 0x01, (char*)"TRIG_IN2_CTL"},      //TRIG_IN2_CTL,
    {   0x74, 0x1A,  0x30,  true, 0x01, (char*)"INVERT_DATA"  },      //INVERT_DATA,
    {   0x75, 0x1A,  0x31,  true, 0x06, (char*)"PAT_CONFIG"   },      //PAT_CONFIG,
    {   0x76, 0x1A,  0x32,  true, 0x02, (char*)"MBOX_ADDRESS"   },      //MBOX_ADDRESS,
    {   0x77, 0x1A,  0x33,  true, 0x01, (char*)"MBOX_CONTROL"   },      //MBOX_CONTROL,
    {   0x78, 0x1A,  0x34,  true, 0x0C, (char*)"MBOX_DATA"    },      //MBOX_DATA,
    {   0x7F, 0x1A,  0x39,  true, 0x01, (char*)"SPLASH_LOAD"   },      //SPLASH_LOAD,
    {   0x48, 0x08,  0x07,  true, 0x03, (char*)"GPCLK_CONFIG"   },      //GPCLK_CONFIG,
    {   0x1A, 0x12,  0x04,  true, 0x0C, (char*)"TPG_COLOR"   },      //TPG_COLOR,
    {   0x4E, 0x1A,  0x13,  true, 0x05, (char*)"PWM_CAPTURE_READ"   },     //PWM_CAPTURE_READ,
    {   0x4F, 0x1A,  0x4F,  true, 0x05, (char*)"I2C_PASSTHRU"},     //I2C_PASSTHRU,
    {	0x2A, 0x1A,  0x2A,  true, 0x6,  (char*)"PATMEM_LOAD_INIT_MASTER"},//PATMEM_LOAD_INIT_MASTER
    {   0x2B, 0x1A,  0x2B,  false, 0x00,(char*)"PATMEM_LOAD_DATA_MASTER"},//PATMEM_LOAD_DATA_MASTER
    {   0x2C, 0x1A,  0x2C,  true, 0x6,  (char*)"PATMEM_LOAD_INIT_SLAVE"},//PATMEM_LOAD_INIT_SLAVE
    {   0x2D, 0x1A,  0x2D,  false, 0x00,(char*) "PATMEM_LOAD_DATA_SLAVE" },//PATMEM_LOAD_DATA_SLAVE
    {   0x3A, 0x1A,  0x14,  false, 0x1, (char*)"BATCHFILE_NAME"}, //BATCHFILE_NAME
    {   0x3B, 0x1A,  0x15,  false, 0x1, (char*)"BATCHFILE_EXECUTE"}, // BATCHFILE_EXECUTE
    {   0x3C, 0x1A,  0x16,  false, 0x4, (char*) "DELAY"}, //DELAY
    {   0x00, 0x1A,  0x5B,  false, 0x05,(char*)"DEBUG"}, //DEBUG
    {   0x45, 0x1A,  0x4E,  true, 0x05, (char*)"I2C_CONFIG"}, //I2C_CONFIG
    {   0x06, 0x11,  0x00,  true, 0x06, (char*)"CURTAIN_COLOR"},//CURTAIN_COLOR
    {   0x0C, 0x1A,  0x01,  true, 0x01, (char*)"VIDEO_CONT_SEL"},//VIDEO_CONT_SEL(IT6535_POWER_MODE)
    {   0x32, 0x01,  0x00,  false, 0x01,(char*)"READ_ERROR_CODE"},//READ_ERROR_CODE
    {   0x33, 0x01,  0x01,  false, 0x80,(char*)"READ_ERROR_MSG"},//READ_ERROR_MSG
    {   0x12, 0x02,  0x06,  false, 0x21,(char*)"READ_FRMW_VERSION"},//READ_FRMW_VERSION
    {   0x60, 0x1A,  0x40,  true, 0x02, (char*)"DMD_BLOCKS"   },      //DMD_BLOCK,
    {   0x0D, 0x02,  0x01,  true, 0x01, (char*)"DMD_IDLE"   },     // BL_MASTER_SLAVE,
    {   0x01, 0x00,  0x00,  false, 0x00,(char*)"BL_STATUS"   },     //BL_STATUS
    {   0x23, 0x00,  0x23,  false, 0x01,(char*)"BL_SPL_MODE"   },     //BL_SPL_MODE
    {   0x0C, 0x00,  0x15,  false, 0x01,(char*)"BL_GET_MANID"   },     //BL_GET_MANID,
    {   0x0D, 0x00,  0x15,  false, 0x01,(char*) "BL_GET_DEVID"   },     //BL_GET_DEVID,
    {   0x00, 0x00,  0x15,  false, 0x01,(char*) "BL_GET_CHKSUM"   },     //BL_GET_CHKSUM,
    {   0x29, 0x00,  0x29,  false, 0x04,(char*) "BL_SET_SECTADDR"   },     //BL_SET_SECTADDR,
    {   0x28, 0x00,  0x28,  false, 0x00,(char*) "BL_SECT_ERASE"   },     //BL_SECT_ERASE,
    {   0x2C, 0x00,  0x2C,  false, 0x04,(char*) "BL_SET_DNLDSIZE"   },     //BL_SET_DNLDSIZE,
    {   0x25, 0x00,  0x25,  false, 0x00,(char*) "BL_DNLD_DATA"   },     //BL_DNLD_DATA,
    {   0x2F, 0x00,  0x2F,  false, 0x01,(char*) "BL_FLASH_TYPE"   },     //BL_FLASH_TYPE,
    {   0x26, 0x00,  0x26,  false, 0x00,(char*) "BL_CALC_CHKSUM"   },     //BL_CALC_CHKSUM,
    {   0x30, 0x00,  0x30,  false, 0x01,(char*) "BL_PROG_MODE"   },     //BL_PROG_MODE,
    {   0x31, 0x00,  0x31,  false, 0x01,(char*) "BL_MASTER_SLAVE"   },     // BL_MASTER_SLAVE,
    {   0x00, 0x00,  0x00,  false, 0x0, (char*) "END"},	// Keep this to identify end of command list.
};

static unsigned char seqNum=0;
static unsigned char PatLut[12][512];
static unsigned int PatLutIndex = 0;
void (*mainWindowCallback) (char*) = NULL;
 ...
 */
{
    ...
    {
        int i = 0;
        while(strcmp(CmdList[i].name, "END"))
        {
            unsigned short cmd = CmdList[i].CMD3 | CmdList[i].CMD2 << 8;
            if(cmd == pMsg->text.cmd)
                break;
            i++;
        }
        if (CmdList[i].batchUpdateEnable)
        {
            char msgStr[2560];
            int len;
            len = sprintf(msgStr, "%s: ",CmdList[i].name);
            //	   for (int j = 0; j < CmdList[i].len; j++)
            for (int j = 0; j < (pMsg->head.length - 2); j++)
                len+= sprintf(msgStr + len, "0x%x ", pMsg->text.data[2 + j]);
            mainWindowCallback(msgStr);
        }
    }
