...
namespace PhaseSolverType {
    FourFloorFourStepMaster_GPU::FourFloorFourStepMaster_GPU(std::vector<cv::Mat>& imgs, const dim3 block_) :
        block(block_), currentFrame(0){
        if (imgs.size() > 0) {
    ...
    void FourFloorFourStepMaster_GPU::getUnwrapPhaseImg(std::vector<cv::cuda::GpuMat>& unwrapImg, cv::cuda::Stream& pStream) {
        /*
        if(currentFrame == 0)
    ...
    void FourFloorFourStepMaster_GPU::changeSourceImg(std::vector<cv::Mat>& imgs, cv::cuda::Stream& stream) {
        rows = imgs[0].rows;
        cols = imgs[0].cols;
    ...
    void FourFloorFourStepMaster_GPU::changeSourceImg(std::vector<cv::cuda::GpuMat>& imgs) {
        rows = imgs[0].rows;
        cols = imgs[0].cols;
    ...
    void FourFloorFourStepMaster_GPU::getTextureImg(std::vector<cv::cuda::GpuMat>& textureImg) {
        textureImg.clear();
        textureImg.resize(1, cv::cuda::GpuMat(conditionImg_device.size(), conditionImg_device.type()));
