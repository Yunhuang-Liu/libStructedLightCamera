...
namespace PhaseSolverType {
    FourStepSixGrayCodeMaster_CPU::FourStepSixGrayCodeMaster_CPU(const cv::Mat& refImg, const int threads_) :
        threads(threads_),
        refAbsImg(refImg){
    }
    ...
    void FourStepSixGrayCodeMaster_CPU::caculateAverageImgs(){
        ...
        for(int i=0;i<threads-1;i++){
            tasks[i] = std::thread(&FourStepSixGrayCodeMaster_CPU::dataInit_Thread_SIMD,this,rows_,cols_,cv::Point2i(rows*i,rows*(i+1)));
        }
        tasks[threads-1] = std::thread(&FourStepSixGrayCodeMaster_CPU::dataInit_Thread_SIMD,this,rows_,cols_,cv::Point2i(rows*(threads-1),rows_));
    ...
    void FourStepSixGrayCodeMaster_CPU::dataInit_Thread_SIMD(const int rows,const int cols,const cv::Point2i region){
        __m256 img_0_data;
        __m256 img_1_data;
        __m256 img_2_data;
        __m256 img_3_data;
        ...
        for(int i=region.x;i<region.y;i++){
            ...
            for(int j=0;j<cols;j+=8){
                ...
                average_data = _mm256_div_ps(_mm256_add_ps(_mm256_add_ps(img_0_data,img_1_data),_mm256_add_ps(img_2_data,img_3_data)),value_4_data);
                i3_i1_data = _mm256_sub_ps(img_3_data,img_1_data);
                i0_i2_data = _mm256_sub_ps(img_0_data,img_2_data);
                condition_data = _mm256_div_ps(_mm256_sqrt_ps(_mm256_add_ps(_mm256_mul_ps(i3_i1_data,i3_i1_data),_mm256_mul_ps(i0_i2_data,i0_i2_data))),value_2_data);
    ...
    void FourStepSixGrayCodeMaster_CPU::mutiplyThreadUnwrap(const int rows,const int cols,const cv::Point2i region,cv::Mat& absolutePhaseImg){
        ...
        for (int i = region.x; i < region.y; i++)
        {
            ...
            for (int j = 0; j < cols; j+=8)
            {
                ...
                K2 = _mm256_floor_ps(_mm256_div_ps(_mm256_add_ps(_mm256_add_ps(_mm256_add_ps(_mm256_add_ps(_mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(bit5,leftMove_5_),_mm256_mul_ps(bit4,leftMove_4_)),_mm256_mul_ps(bit3,leftMove_3_)),
                                            _mm256_mul_ps(bit2,leftMove_2_)),_mm256_mul_ps(bit1,leftMove_1_)),bit0),add_1_),div_2_));
                ...
                K1 = _mm256_add_ps(_mm256_add_ps(_mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(bit4,leftMove_4_),_mm256_mul_ps(bit3,leftMove_3_)),_mm256_mul_ps(bit2,leftMove_2_)),
                            _mm256_mul_ps(bit1,leftMove_1_)),bit0);
    ...
    void FourStepSixGrayCodeMaster_CPU::getUnwrapPhaseImg(cv::Mat& absolutePhaseImg){
        ...
        int rows = rows_ / threads;
        for(int i=0;i<threads-1;i++){
            tasks[i] = std::thread(&FourStepSixGrayCodeMaster_CPU::mutiplyThreadUnwrap,this,rows_,cols_,cv::Point2i(rows*i,rows*(i+1)),std::ref(absolutePhaseImg));
        }
        tasks[threads-1] = std::thread(&FourStepSixGrayCodeMaster_CPU::mutiplyThreadUnwrap,this,rows_,cols_,cv::Point2i(rows*(threads-1),rows_),std::ref(absolutePhaseImg));
        for(int i=0;i<threads;i++){
            tasks[i].join();
        }
    }
    ...
    void FourStepSixGrayCodeMaster_CPU::atan2M(const cv::Mat& lhs,const cv::Mat& rhs,cv::Mat& wrapImg,const int threads){
        ...
        for(int i=0;i<threads - 1;i++){
            tasks[i] = std::thread(&PhaseSolverType::FourStepSixGrayCodeMaster_CPU::SIMD_WrapImg,this,std::ref(lhs),std::ref(rhs),cv::Point2i(rows*i,rows*(i+1)),std::ref(wrapImg));
        }
        tasks[threads - 1] = std::thread(&PhaseSolverType::FourStepSixGrayCodeMaster_CPU::SIMD_WrapImg,this,std::ref(lhs),std::ref(rhs),cv::Point2i(rows*(threads-1),lhs.rows),std::ref(wrapImg));
    ...
    void FourStepSixGrayCodeMaster_CPU::SIMD_WrapImg(const cv::Mat& lhs,const cv::Mat& rhs,const cv::Point2i& region,cv::Mat& wrapImg){
        __m256 lhs_Up;
        __m256 rhs_Down;
    ...
    void FourStepSixGrayCodeMaster_CPU::getWrapPhaseImg(cv::Mat& wrapImg_, cv::Mat& conditionImg_) {
        std::vector<std::thread> convertFloatThreads(imgs.size());
        for (int i = 0; i < convertFloatThreads.size(); i++) {
            convertFloatThreads[i] = std::thread([&, i] {
                imgs[i].convertTo(imgs[i], CV_32FC1);
