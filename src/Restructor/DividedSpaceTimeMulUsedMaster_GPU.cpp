...
namespace PhaseSolverType {
    DividedSpaceTimeMulUsedMaster_GPU::DividedSpaceTimeMulUsedMaster_GPU(std::vector<cv::Mat> &imgs,const cv::Mat& refImgWhite, const dim3 block_) : refImgWhite_device(refImgWhite),block(block_){
    ...
    void DividedSpaceTimeMulUsedMaster_GPU::getWrapPhaseImg(cv::cuda::Stream& stream) {
        PhaseSolverType::cudaFunc::solvePhasePrepare_DevidedSpace(img1_4_device, img1_2_device, img1_3_device, img1_1_device,
            ...
            wrapImg3_1_device, wrapImg3_2_device, wrapImg3_3_device, conditionImg_3_device,
            wrapImg4_1_device, wrapImg4_2_device, wrapImg4_3_device, conditionImg_4_device,
            floor_K_device,block, stream);
    }

    void DividedSpaceTimeMulUsedMaster_GPU::getUnwrapPhaseImg(std::vector<cv::cuda::GpuMat>& unwrapImg, cv::cuda::Stream& stream) {
        unwrapImg.resize(4, cv::cuda::GpuMat(rows, cols, CV_32FC1));
        getWrapPhaseImg(stream);
        //cudaDeviceSynchronize();
        PhaseSolverType::cudaFunc::solvePhase_DevidedSpace(refImgWhite_device, rows, cols,
            wrapImg1_1_device, wrapImg1_2_device, wrapImg1_3_device, conditionImg_1_device, unwrapImg[0],
            wrapImg2_1_device, wrapImg2_2_device, wrapImg2_3_device, conditionImg_2_device, unwrapImg[1],
            wrapImg3_1_device, wrapImg3_2_device, wrapImg3_3_device, conditionImg_3_device, unwrapImg[2],
            wrapImg4_1_device, wrapImg4_2_device, wrapImg4_3_device, conditionImg_4_device, unwrapImg[3], floor_K_device,block,stream);
        //cudaDeviceSynchronize();
    }

    DividedSpaceTimeMulUsedMaster_GPU::DividedSpaceTimeMulUsedMaster_GPU(const cv::Mat &refImgWhite_, const dim3 block_) : refImgWhite_device(refImgWhite_), block(block_){
    ...
    void DividedSpaceTimeMulUsedMaster_GPU::changeSourceImg(std::vector<cv::Mat> &imgs){
        img1_1_device.upload(imgs[0]);
        img1_2_device.upload(imgs[1]);
        img1_3_device.upload(imgs[2]);
        img1_4_device.upload(imgs[3]);
    ...
    void DividedSpaceTimeMulUsedMaster_GPU::changeSourceImg(std::vector<cv::Mat>& imgs, cv::cuda::Stream& stream) {
