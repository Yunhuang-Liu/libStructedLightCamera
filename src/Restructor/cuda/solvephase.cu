...
namespace PhaseSolverType {
    namespace cudaFunc {
        ...
        __global__ void kMeansCluster(const cv::cuda::PtrStep<float> src, const cv::cuda::PtrStep<float> conditionImg,
            const int rows, const int cols, cv::cuda::PtrStep<float> threshodVal, cv::cuda::PtrStep<float> threshodAdd, cv::cuda::PtrStep<float> count) {
            const int x = blockIdx.x * blockDim.x + threadIdx.x;
            const int y = blockIdx.y * blockDim.y + threadIdx.y;
            if (x < cols && y < rows) {
        ...
        __global__ void medianFilter(const cv::cuda::PtrStep<float> src, const cv::cuda::PtrStep<float> conditionImgCopy,
            const int kSize, const int rows, const int cols, cv::cuda::PtrStep<float> dst, cv::cuda::PtrStep<float> conditionImg) {
            const int x = blockIdx.x * blockDim.x + threadIdx.x;
            const int y = blockIdx.y * blockDim.y + threadIdx.y;
            if (x < cols && y < rows) {
        ...
        __global__ void prepare_FourFloorFourStep(const cv::cuda::PtrStep<uchar> shift_0_, const cv::cuda::PtrStep<uchar> shift_1_, const cv::cuda::PtrStep<uchar> shift_2_, const cv::cuda::PtrStep<uchar> shift_3_,
            const cv::cuda::PtrStep<uchar> gray_0_, const cv::cuda::PtrStep<uchar> gray_1_,
            const int rows, const int cols, cv::cuda::PtrStep<float> wrapImg, cv::cuda::PtrStep<float> conditionImg, cv::cuda::PtrStep<float> kFloorImg_0_, cv::cuda::PtrStep<float> kFloorImg_1_, const bool isOdd) {
            const int x = blockIdx.x * blockDim.x + threadIdx.x;
            const int y = blockIdx.y * blockDim.y + threadIdx.y;
            if (x < cols && y < rows) {
        ...
        __global__ void caculateKFloorImg(const cv::cuda::PtrStep<float> kFloorImg_0_, const cv::cuda::PtrStep<float> kFloorImg_1_, const cv::cuda::PtrStep<float> conditionImg,
            const cv::cuda::PtrStep<float> threshodVal, const int rows, const int cols, cv::cuda::PtrStep<uchar> kFloorImg) {
            const int x = blockIdx.x * blockDim.x + threadIdx.x;
            const int y = blockIdx.y * blockDim.y + threadIdx.y;
            if (x < cols && y < rows) {
                if (conditionImg.ptr(y)[x] < 10.f) {
        ...
        __global__ void getUnwrapImg_FourFloorFourStep(const cv::cuda::PtrStep<uchar> kFloorImg, const cv::cuda::PtrStep<float> conditionImg, const cv::cuda::PtrStep<float> wrapImg,
            const int rows, const int cols, cv::cuda::PtrStep<float> unwrapImg) {
            const int x = blockIdx.x * blockDim.x + threadIdx.x;
            const int y = blockIdx.y * blockDim.y + threadIdx.y;
            if (x < cols && y < rows) {
                if (conditionImg.ptr(y)[x] < 10.f) {
        ...
        __global__ void atan3M_ShiftGrayFourFrame(const cv::cuda::PtrStep<uchar> shift_0_0_, const cv::cuda::PtrStep<uchar> shift_0_1_, const cv::cuda::PtrStep<uchar> shift_0_2_,
            const cv::cuda::PtrStep<uchar> shift_1_0_, const cv::cuda::PtrStep<uchar> shift_1_1_, const cv::cuda::PtrStep<uchar> shift_1_2_,
            const cv::cuda::PtrStep<uchar> shift_2_0_, const cv::cuda::PtrStep<uchar> shift_2_1_, const cv::cuda::PtrStep<uchar> shift_2_2_,
            const cv::cuda::PtrStep<uchar> shift_3_0_, const cv::cuda::PtrStep<uchar> shift_3_1_, const cv::cuda::PtrStep<uchar> shift_3_2_,
            const cv::cuda::PtrStep<uchar> gray_0_, const cv::cuda::PtrStep<uchar> gray_1_, const cv::cuda::PtrStep<uchar> gray_2_, const cv::cuda::PtrStep<uchar> gray_3_,
            const int rows, const int cols,
            cv::cuda::PtrStep<float> wrapImg_1_, cv::cuda::PtrStep<float> conditionImg_1_,
        ...
        __global__ void getUnwrapImg_ShiftGray(const cv::cuda::PtrStep<float> absolutImgWhite, const int rows, const int cols,
            ...
            if (x < cols && y < rows) {
                ...
                if (conditionImg_1_.ptr(y)[x] < 5.0) {
                ...
                if (conditionImg_2_.ptr(y)[x] < 5.0) {
        ...
        __global__ void getUnwrapImg_ShiftGrayFourFrame(const cv::cuda::PtrStep<float> absolutImgWhite, const int rows, const int cols,
            const cv::cuda::PtrStep<float> wrapImg_1_, const cv::cuda::PtrStep<float> conditionImg_1_, cv::cuda::PtrStep<float> unwrapImg_1_,
            const cv::cuda::PtrStep<float> wrapImg_2_, const cv::cuda::PtrStep<float> conditionImg_2_, cv::cuda::PtrStep<float> unwrapImg_2_,
            const cv::cuda::PtrStep<float> wrapImg_3_, const cv::cuda::PtrStep<float> conditionImg_3_, cv::cuda::PtrStep<float> unwrapImg_3_, 
            const cv::cuda::PtrStep<float> wrapImg_4_, const cv::cuda::PtrStep<float> conditionImg_4_, cv::cuda::PtrStep<float> unwrapImg_4_, 
            cv::cuda::PtrStep<uchar> floor_K) {
            const int x = blockIdx.x * blockDim.x + threadIdx.x;
            const int y = blockIdx.y * blockDim.y + threadIdx.y;
            const float CV_2PI_DIV_3 = CV_2PI / 3;
            if (x < cols && y < rows) {
                ...
                if (conditionImg_3_.ptr(y)[x] < 5.0) {
                    ...
                    if ((CV_2PI / 3 < wrapImg_3_.ptr(y)[x]) || (x < refValue_3_compare && cuda::std::abs(wrapImg_3_.ptr(y)[x]) < CV_2PI / 3)) {
                ...
                if (conditionImg_4_.ptr(y)[x] < 5.0) {
                    ...
                    if ((CV_2PI / 3 < wrapImg_4_.ptr(y)[x]) || (x < refValue_4_compare && cuda::std::abs(wrapImg_4_.ptr(y)[x]) < CV_2PI / 3)) {
        ...
        void solvePhasePrepare_ShiftGrayFourFrame(const cv::cuda::GpuMat& shift_0_0_, const cv::cuda::GpuMat& shift_0_1_, const cv::cuda::GpuMat& shift_0_2_,
            ...
            cudaStream_t stream = cv::cuda::StreamAccessor::getStream(cvStream);
            dim3 grid((cols + block.x - 1) / block.x, (rows + block.y - 1) / block.y);
            atan3M_ShiftGrayFourFrame << <grid, block, 0, stream >> > (shift_0_0_, shift_0_1_, shift_0_2_,
                shift_1_0_, shift_1_1_, shift_1_2_,
        ...
        void solvePhase_ShiftGrayFourFrame(const cv::cuda::GpuMat& absolutImgWhite, const int rows, const int cols,
            ...
            cudaStream_t stream = cv::cuda::StreamAccessor::getStream(cvStream);
            dim3 grid((cols + block.x - 1) / block.x, (rows + block.y - 1) / block.y);
            getUnwrapImg_ShiftGrayFourFrame << <grid, block, 0, stream >> > (absolutImgWhite, rows, cols,
                wrapImg_1_, conditionImg_1_, unwrapImg_1_,
        ...
        void solvePhasePrepare_FourFloorFourStep(const cv::cuda::GpuMat& shift_0_, const cv::cuda::GpuMat& shift_1_, const cv::cuda::GpuMat& shift_2_, const cv::cuda::GpuMat& shift_3_,
            ...
            prepare_FourFloorFourStep << <grid, block, 0, stream >> > (shift_0_, shift_1_, shift_2_, shift_3_,
                gray_0_, gray_1_,
                rows, cols, wrapImg, conditionImg, kFloorImg_0_, kFloorImg_1_, isOdd);
            /*
            if(isFirstStart){
            ...
            */
            ...
            if (isFirstStart) {
                medianFilter << <grid, block, 0, stream >> > (kFloorImg_0_, conditionImgCopy,
                    5, rows, cols, medianFilter_0_, conditionImg);
                medianFilter << <grid, block, 0, stream >> > (kFloorImg_1_, conditionImgCopy,
            ...
            for (int i = 0; i < countKmeans; i++) 
                kMeansCluster <<<grid, block, 0, stream >>>(kFloorImg_0_, conditionImg, rows, cols, threshodVal, threshodAdd, count);
            /*
        ...
        void solvePhase_FourFloorFourStep(const cv::cuda::GpuMat& kFloorImg, const cv::cuda::GpuMat& conditionImg, const cv::cuda::GpuMat& wrapImg,
            ...
            cudaStream_t stream = cv::cuda::StreamAccessor::getStream(cvStream);
            dim3 grid((cols + block.x - 1) / block.x, (rows + block.y - 1) / block.y);
            getUnwrapImg_FourFloorFourStep << <grid, block, 0, stream >> > (kFloorImg, conditionImg, wrapImg,
                rows, cols, unwrapImg);
        }
