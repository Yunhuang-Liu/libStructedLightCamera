...
ThreeStepFiveGrayCodeMaster_CPU::ThreeStepFiveGrayCodeMaster_CPU(
    std::vector<cv::Mat>& imgs_, const int threads_) :
    imgs(imgs_),
    threads(threads_) {
...
void ThreeStepFiveGrayCodeMaster_CPU::caculateAverageImgs() {
    ...
    for(int i=0;i<threads-1;i++) {
        tasks[i] = std::thread(
            &ThreeStepFiveGrayCodeMaster_CPU::dataInit_Thread_SIMD,
            this,
            rows_,
            cols_,
...
void ThreeStepFiveGrayCodeMaster_CPU::dataInit_Thread_SIMD(
...
void ThreeStepFiveGrayCodeMaster_CPU::mutiplyThreadUnwrap(
...
void ThreeStepFiveGrayCodeMaster_CPU::getUnwrapPhaseImg(cv::Mat& absolutePhaseImg) {
    ...
    int rows = rows_ / threads;
    for(int i=0;i<threads-1;i++){
        tasks[i] = std::thread(&ThreeStepFiveGrayCodeMaster_CPU::mutiplyThreadUnwrap,
        this,
        rows_,
        cols_,
        cv::Point2i(rows*i,rows*(i+1)),
    ...
    tasks[threads-1] = std::thread(
        &ThreeStepFiveGrayCodeMaster_CPU::mutiplyThreadUnwrap,
        this,
        rows_,
        cols_,
        cv::Point2i(rows*(threads-1),rows_),
...
ThreeStepFiveGrayCodeMaster_CPU::ThreeStepFiveGrayCodeMaster_CPU(
    const int threads_) : threads(threads_), imgs(std::vector<cv::Mat>()){

}

void ThreeStepFiveGrayCodeMaster_CPU::atan3M(const cv::Mat& firstStep, 
    const cv::Mat& secondStep, const cv::Mat& thirdStep, 
    cv::Mat& wrapImg, const int threads) {
    std::vector<std::thread> tasks;
    tasks.resize(threads);
    int rows = firstStep.rows / threads;
    for(int i=0;i<threads-1;i++) {
        tasks[i] = std::thread(
            &ThreeStepFiveGrayCodeMaster_CPU::SIMD_WrapImg,
            this,
            std::ref(firstStep),
            std::ref(secondStep), 
            std::ref(thirdStep), 
...
void ThreeStepFiveGrayCodeMaster_CPU::SIMD_WrapImg(const cv::Mat& firstStep, 
    const cv::Mat& secondStep, const cv::Mat& thirdStep, 
    const cv::Point2i& region,cv::Mat& wrapImg) {
    __m256 first_step;
    __m256 second_step;
    __m256 third_step;
