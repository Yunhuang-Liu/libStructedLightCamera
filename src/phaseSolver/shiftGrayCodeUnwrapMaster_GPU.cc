#include "../../include/phaseSolver/shiftGrayCodeUnwrapMaster_GPU.h"
...
namespace sl {
    namespace phaseSolver {
        ...
        void ShiftGrayCodeUnwrapMaster_GPU::getUnwrapPhaseImg(
    ...
    */
        ...
        void ShiftGrayCodeUnwrapMaster_GPU::changeSourceImg(
                std::vector<cv::Mat> &imgs, cv::cuda::Stream &cvStream) {
            if (0 == rows) {
                rows = imgs[0].rows;
                cols = imgs[0].cols;
                wrapImg1_device = cv::cuda::createContinuous(rows, cols, CV_32FC1);
        ...
        ShiftGrayCodeUnwrapMaster_GPU::ShiftGrayCodeUnwrapMaster_GPU(
                const dim3 block_, const cv::Mat &refImgWhite) : rows(0), cols(0), block(block_), refImgWhite_device(refImgWhite) {
            //refImgWhite_device.upload(refImgWhite);
        }

        void ShiftGrayCodeUnwrapMaster_GPU::getTextureImg(
