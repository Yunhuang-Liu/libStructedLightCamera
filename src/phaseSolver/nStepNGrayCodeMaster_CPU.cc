#include "../../include/phaseSolver/nStepNGrayCodeMaster_CPU.h"
...
namespace sl {
    namespace phaseSolver {
        ...
        void NStepNGrayCodeMaster_CPU::dataInit_Thread_SIMD(
            ...
            for (int i = region.x; i < region.y; i++) {
                ...
                for (int j = 0; j < cols; j += 8) {
                    ...
                    __m256 dataLoad = _mm256_load_ps(&ptrWrapImg[j]);
                    for (int d = 0; d < 8; d++) {
			cosData[d] = cos(dataLoad[d]);
		    }
        ...
        void NStepNGrayCodeMaster_CPU::SIMD_WrapImg(
            ...
            __m256 cosShiftNow = _mm256_set1_ps(0);
            __m256 sinShiftNow = _mm256_set1_ps(0); 
            __m256 atan2Val = _mm256_set1_ps(0);            
            for (size_t i = region.x; i < region.y; i++) {
                ...
                for (size_t j = 0; j < cols; j += 8) {
                    ...
                    for (int step = 0; step < shiftStep; ++step) {
                        ...
                        for (int d = 0; d < 8; d++) {
			    sinShiftNow[d] = sin(shiftNow[d]);
			    cosShiftNow[d] = cos(shiftNow[d]);
			}
                        sinPartial = _mm256_add_ps(sinPartial, _mm256_mul_ps(datas[step], sinShiftNow));
                        cosPartial = _mm256_add_ps(cosPartial, _mm256_mul_ps(datas[step], cosShiftNow));
                    }
                    for (int d = 0; d < 8; d++) {
			atan2Val[d] = atan2f(sinPartial[d], cosPartial[d]);
		    }
                    result = _mm256_mul_ps(dataCounter1, atan2Val);
