#include "../../include/phaseSolver/fourStepSixGrayCodeMaster_GPU.h"
    ...
    namespace phaseSolver {
        ...
        void FourStepSixGrayCodeMaster_GPU::getUnwrapPhaseImg(
            ...
            phaseSolver::cudaFunc::solvePhase_FourStepSixGray(
                    ...
                    rows, cols,
                    averageImg_device, conditionImg_device, wrapImg_device, unwrapImg[0],
                    block, pStream);
        }
        ...
        void FourStepSixGrayCodeMaster_GPU::changeSourceImg(
        ...
        FourStepSixGrayCodeMaster_GPU::FourStepSixGrayCodeMaster_GPU(
                const dim3 block_) : block(block_) {
        }

        void FourStepSixGrayCodeMaster_GPU::getTextureImg(
                std::vector<cv::cuda::GpuMat> &textureImg) {
            textureImg.clear();
            textureImg.resize(1, cv::cuda::GpuMat(averageImg_device.size(),
