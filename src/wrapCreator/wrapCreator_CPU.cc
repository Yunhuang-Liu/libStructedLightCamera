#include "../../include/wrapCreator/wrapCreator_CPU.h"
...
namespace sl {
    namespace wrapCreator {
        ...
        void WrapCreator_CPU::thread_ThreeStepWrap(
		    ...
		    __m256 upper = _mm256_mul_ps(diffFT, dataSqrtThree);
                    __m256 wrapVal = _mm256_set1_ps(0.f);
                    for(int i = 0; i < 8; ++i) {
                    	wrapVal[i] = std::atan2(upper[i], diffSSFT[i]);
                    }
        ...
        void WrapCreator_CPU::thread_FourStepWrap(
            ...
            for (int i = 0; i < region.height; i++) {
                ...
                for (int j = 0; j < region.width; j += 8) {
                    ...
                    __m256 wrapVal = _mm256_set1_ps(0.f);
                    for(int i = 0; i < 8; ++i) {
                    	wrapVal[i] = std::atan2(diffFS[i], diffFT[i]);
                    }
                    _mm256_store_ps(&ptr_wrapImg[j], wrapVal);
