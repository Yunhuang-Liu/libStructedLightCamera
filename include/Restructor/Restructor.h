...
namespace RestructorType {
    ...
    namespace cudaFunc {
        ...
        void depthColorMap(const cv::cuda::GpuMat& leftImg_, const cv::cuda::GpuMat& rightImg_, const cv::cuda::GpuMat& colorImg_, const int rows, const int cols,
                           ...
                           cv::cuda::GpuMat& depthMap, cv::cuda::GpuMat& mapColor, const dim3 block, cv::cuda::Stream& pStream);
    }
    #endif
    /** \brief 重建器 */
    class Restructor{
        public:
             ...
             */
            virtual void restruction(const cv::Mat& leftAbsImg, const cv::Mat& rightAbsImg, const cv::Mat& colorImg,
                             cv::Mat& depthImgOut, cv::Mat& colorImgOut) = 0;
            #ifdef CUDA
            /**
             * @brief 获取深度和纹理图
             * @param index 输入，图片索引
             ...
             */
            virtual void download(const int index, cv::cuda::GpuMat& depthImg, cv::cuda::GpuMat& colorImg) = 0;
            /**
             * @brief 重建
             * @param leftAbsImg 输入，左绝对相位
             * @param rightAbsImg 输入，右绝对相位
             ...
             * @param stream 输入，异步流
             */
            virtual void restruction(const cv::cuda::GpuMat& leftAbsImg, const cv::cuda::GpuMat& rightAbsImg, const cv::Mat& colorImg,
                                     const int sysIndex, cv::cuda::Stream& stream) = 0;
            #endif
        protected:
            /**
             * @brief 获取深度和纹理，便于梳理
