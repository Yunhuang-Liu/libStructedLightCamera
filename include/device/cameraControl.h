...
#include "./camera/cammeraUnilty.h"
    ...
    namespace device {
        ...
        public:
            ...
            */
             ...
             * @brief 获取左相机图片队列
             */
             inline std::queue<cv::Mat>& leftImgQueue() { return cameraLeft->imgQueue;}
             /**
             * @brief 获取右相机图片队列
             */
             inline std::queue<cv::Mat>& rightImgQueue() { return cameraRight->imgQueue;}
             /**
             * @brief 获取彩色相机图片队列
             */
             inline std::queue<cv::Mat>& colorImgQueue() { return cameraColor->imgQueue;}
             /**
              * @brief 加载预制图片固件(*.bin)
              * @param fileName 输入，文件名
              */
             inline void loadPatternData(const std::string fileName) { return projector->loadPatternData(fileName);}
        private:
            /** \左相机 **/
